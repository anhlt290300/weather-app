<template>
  <div class="w-full p-2 border border-black rounded flex items-center justify-center z-10">
    <div
      ref="mapContainer"
      class="w-full aspect-video border border-black/60 justify-between"
    ></div>
  </div>
</template>
  ...
  if (navigator.geolocation) {
    ...
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      leaflet
        ...
        .addTo(map.value)
      const marker_item = leaflet
        .marker([lat.value, lon.value], {
          icon: customIcon(lat.value, lon.value)
        })
        .addTo(map.value)

      marker_item.on('click', () => {
        map.value.setView([lat.value, lon.value], 16)
      })
      marker.value.push(marker_item)
    })
  }
})
